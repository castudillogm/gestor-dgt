...
export function parseDgtExcel(buffer) {
  ...
  // Iteramos sobre todas las hojas del Excel
  for (const sheetName of workbook.SheetNames) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (firstCell.includes('Vehículos Pesados') && firstCell.includes('Maquinaria agrícola')) {
        console.log(`ENTRÓ A PESADOS en la hoja ${sheetName}!`, firstCell);
        isPesadosSection = true;
        continue;
      }
      
      if (i < 5) console.log(`Hoja ${sheetName} - Fila`, i, firstCell, row);
